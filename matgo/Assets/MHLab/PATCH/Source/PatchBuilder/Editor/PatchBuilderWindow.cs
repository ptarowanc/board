...
public class PatchBuilderWindow : EditorWindow
{
    ...
    {
        if(EditorPrefs.HasKey("MATGO_PATCH_SFTP_prefsExist"))
        {
            if(EditorPrefs.HasKey("MATGO_PATCH_SFTP_isSFTP"))
            {
                _isSFTP = EditorPrefs.GetBool("MATGO_PATCH_SFTP_isSFTP");
            }
            if (EditorPrefs.HasKey("MATGO_PATCH_SFTP_host"))
            {
                _uploadHost = EditorPrefs.GetString("MATGO_PATCH_SFTP_host");
            }
            if (EditorPrefs.HasKey("MATGO_PATCH_SFTP_host"))
            {
                _uploadHostPort = EditorPrefs.GetInt("MATGO_PATCH_SFTP_port");
            }
            if (EditorPrefs.HasKey("MATGO_PATCH_SFTP_username"))
            {
                _uploadUsername = EditorPrefs.GetString("MATGO_PATCH_SFTP_username");
            }
            if (EditorPrefs.HasKey("MATGO_PATCH_SFTP_password"))
            {
                _uploadPassword = EditorPrefs.GetString("MATGO_PATCH_SFTP_password");
            }
            if (EditorPrefs.HasKey("MATGO_PATCH_SFTP_buildsRemotePath"))
            {
                _buildsRemotePath = EditorPrefs.GetString("MATGO_PATCH_SFTP_buildsRemotePath");
            }
            if (EditorPrefs.HasKey("MATGO_PATCH_SFTP_patchesRemotePath"))
            {
                _patchesRemotePath = EditorPrefs.GetString("MATGO_PATCH_SFTP_patchesRemotePath");
            }
			if (EditorPrefs.HasKey("MATGO_PATCH_SFTP_patcherRemotePath"))
			{
				_patcherRemotePath = EditorPrefs.GetString("MATGO_PATCH_SFTP_patcherRemotePath");
			}
        }
    }
    ...
    {
        EditorPrefs.SetBool("MATGO_PATCH_SFTP_prefsExist", true);
        EditorPrefs.SetBool("MATGO_PATCH_SFTP_isSFTP", _isSFTP);
        EditorPrefs.SetString("MATGO_PATCH_SFTP_host", _uploadHost);
        EditorPrefs.SetInt("MATGO_PATCH_SFTP_port", _uploadHostPort);
        EditorPrefs.SetString("MATGO_PATCH_SFTP_username", _uploadUsername);
        EditorPrefs.SetString("MATGO_PATCH_SFTP_password", _uploadPassword);
        EditorPrefs.SetString("MATGO_PATCH_SFTP_buildsRemotePath", _buildsRemotePath);
		EditorPrefs.SetString("MATGO_PATCH_SFTP_patchesRemotePath", _patchesRemotePath);
		EditorPrefs.SetString("MATGO_PATCH_SFTP_patcherRemotePath", _patcherRemotePath);
    }

    private void DeleteEditorPrefs()
    {
        EditorPrefs.DeleteKey("MATGO_PATCH_SFTP_prefsExist");
        EditorPrefs.DeleteKey("MATGO_PATCH_SFTP_isSFTP");
        EditorPrefs.DeleteKey("MATGO_PATCH_SFTP_host");
        EditorPrefs.DeleteKey("MATGO_PATCH_SFTP_port");
        EditorPrefs.DeleteKey("MATGO_PATCH_SFTP_username");
        EditorPrefs.DeleteKey("MATGO_PATCH_SFTP_password");
        EditorPrefs.DeleteKey("MATGO_PATCH_SFTP_buildsRemotePath");
		EditorPrefs.DeleteKey("MATGO_PATCH_SFTP_patchesRemotePath");
		EditorPrefs.DeleteKey("MATGO_PATCH_SFTP_patcherRemotePath");
