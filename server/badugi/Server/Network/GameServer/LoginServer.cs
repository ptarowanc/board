...
namespace Server.Network
{
    class LoginServer : GameServer
    {
        ...
        public LoginServer(BadugiService s, UnityCommon.Server t, ushort portnum) : base(s, t, portnum, 0)
        {
        }
        ...
        protected override void BeforeServerStart(out StartOption param)
        {
            ...
            // Client Stub
            {
                ...
                Stub.RequestLauncherLogin = RequestLauncherLogin;
                Stub.RequestLauncherLogout = RequestLauncherLogout;
        ...
        int TryLogin(string ip, string id, string pass, ref UserData dummy, bool useKey)
        {
            ...
            try
            {
                ...
                if (Data_Player.AdminMemo != null && Data_Player.AdminMemo != "" && Data_Player.AdminMemo.Contains("먹튀"))
                {
                    dummy.Restrict = true;
                }
                ...
                if (Data_Badugi == null)
                {
                    ...
                    Data_Badugi = db.PlayerBadugi.FindAllByUserID(dummy.ID).FirstOrDefault();
                }
                dummy.winCount = Data_Badugi.Win;
                dummy.loseCount = Data_Badugi.Lose;

                if (Data_Badugi.OptionVoiceType != null)
                    dummy.Option1 = Data_Badugi.OptionVoiceType;

                if (Data_Badugi.OptionBetType != null)
                    dummy.Option2 = Data_Badugi.OptionBetType;
                ...
                dynamic Data_Mission = db.PlayerBadugiMission.FindAllByUserID(dummy.ID).FirstOrDefault();
                if (Data_Mission == null)
                {
                    db.PlayerBadugiMission.Insert(UserId: dummy.ID, Point: 0);
                    Data_Mission = db.PlayerBadugiMission.FindAllByUserID(dummy.ID).FirstOrDefault();
                }
                dummy.mileage = Data_Mission.Point;

                db.Player.UpdateById(Id: dummy.ID, LastLoginDateDate: DateTime.Now, LastIP: ip);
        ...
        bool RequestLauncherLogin(RemoteID remote, CPackOption rmiContext, string id, string pass)
        {
            CPlayer rc;
            if (RemoteClients.TryGetValue(remote, out rc) == false) return false;
            string ip = rc.m_ip;
            ...
            Task.Run(async () =>
            {
                ...
                var result = await Task.Run(() =>
                {
                    try
                    {
                        ...
                        key = HexStringFromBytes(sha.ComputeHash(Encoding.UTF8.GetBytes(id + DateTime.Now.ToString() + "vong")));

                        dynamic Data_Password = db.PlayerPassword.FindAllByUserID(Data_Player.Id).FirstOrDefault();

                        if (Data_Password == null)
                        {
                            db.PlayerPassword.Insert(UserId: Data_Player.Id, Password: key, CreatedOnUtc: DateTime.Now);
                        }
                        else
                        {
                            db.PlayerPassword.UpdateByUserId(UserId: Data_Player.Id, Password: key, CreatedOnUtc: DateTime.Now);
                        }
                    ...
                    {
                        Log._log.FatalFormat("RequestLauncherLogin Exception. id:{0}, e:{1}", id, e.ToString());
                    }
                ...
                if (RemoteClients.TryGetValue(remote, out rc) == false)
                {
                    Log._log.WarnFormat("RequestLauncherLogin Cancel. id{0}", id);
                    return;
                }
                ...
                if (result == 1)
                {
                    Proxy.ResponseLauncherLogin(remote, CPackOption.Encrypt, true, nickName, key, 0);
                }
                else
                {
                    byte ResultType;
                    ...
                    Log._log.InfoFormat("RequestLauncherLogin Failure. id:{0}, ResultType:{1}, result:{2}", id, ResultType, result);

                    // 인증 실패를 알려줌
                    Proxy.ResponseLauncherLogin(remote, CPackOption.Encrypt, false, nickName, key, ResultType);
                }
                return;
            });
        ...
        bool RequestLauncherLogout(RemoteID remote, CPackOption rmiContext, string id, string key)
        {
            //Task.Run(() =>
            //{
            CPlayer rc;
            ...
            try
            {
                // key 검사 생략
                {
                    Proxy.ResponseLauncherLogout(remote, CPackOption.Basic);
                }
            }
            catch (Exception e)
            {
                Log._log.FatalFormat("RequestLauncherLogout Failure. id:{0}, e:{1}", id, e.ToString());
                return false;
            }
            //});
            return true;
        ...
        bool RequestGameOptions(RemoteID remote, CPackOption rmiContext, CMessage data)
        {
            ...
            try
            {
                ...
                msg.Read(out OptionBetType);

                Task.Run(() =>
                {
                    try
                    {
                        db.PlayerBadugi.UpdateByUserId(UserId: rc.data.ID, OptionVoiceType: OptionVoiceType, OptionBetType: OptionBetType);
