...
{
#ifdef FB_WORKAROUND
  return (RENDERTARGETVIEW *)(UINTa)render_texture_target;
#else
  GLuint RFramebuffer;
  GLint save;
  ...
  glGetIntegerv(GL_FRAMEBUFFER_BINDING, &save);
  {
    ...
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, (GLint)(UINTa)render_texture_target, 0);
  }
  glBindFramebuffer( GL_FRAMEBUFFER, save );

  return (RENDERTARGETVIEW *)(UINTa)RFramebuffer;
#endif
}

void freerendertargetview_gl( RENDERTARGETVIEW * render_view )
{
#ifndef FB_WORKAROUND
  GLuint RFramebuffer;
  RFramebuffer = (GLint)(UINTa)render_view;
  glDeleteFramebuffers( 1, &RFramebuffer );
#endif
}


#if !defined( __RADIPHONE__ ) && !defined( __RADANDROID__ )
...
#ifdef FB_WORKAROUND // Unity workaround when maximizing a window
    glBindFramebuffer(GL_FRAMEBUFFER, (GLint)(UINTa)rfb);
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, (GLint)(UINTa)render_view, 0);
#else
    glBindFramebuffer(GL_FRAMEBUFFER, (GLint)(UINTa)render_view);
#endif
