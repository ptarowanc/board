...
public class ChannerManager : MonoBehaviour
{
    ...
    void RoomNumberCreate(GameObject parent, int number)
    {
        ...
        //// 등차수열에 사용될 위치값 및 공차
        //float numberX = -165f; //(_num.Length - 1) * -5;
        //float numberY = 125f;
        //float numberZ = 0;
        //float numberD = 22;
        ...
        //{
        //    if (i == 0) parent.transform.Find("Number").GetComponent<Image>().sprite = CSpriteManager.Instance.GetSpriteByName("room_number_0" + _num[i].ToString());
        //    else
        //    {
        //        var _number = Instantiate(parent.transform.Find("Number"));
        //        _number.transform.parent = parent.transform;
        //        _number.localScale = Vector3.one;
        //        _number.GetComponent<Image>().sprite = CSpriteManager.Instance.GetSpriteByName("room_number_0" + _num[i].ToString());
        //        _number.transform.localPosition = new Vector3(numberX + i * numberD, numberY, numberZ);
        //    }
        //}
    }
}