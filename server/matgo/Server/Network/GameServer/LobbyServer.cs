...
namespace Server.Network
{
    class LobbyServer : GameServer
    {
        ...
        public LobbyServer(MatgoService f, UnityCommon.Server t, ushort portnum) : base(f, t, portnum, 0)
        {
        ...
        void Brodcast_Kick_Player(int userId)
        {
            Task.Run(() =>
            {
                try
                {
                    ...
                    Log._log.InfoFormat("Brodcast_Kick_Player success. userId:{0}", userId);
                }
                catch (Exception e)
                {
        ...
        bool RoomLobbyRequestMoveRoom(RemoteID remote, CPackOption rmiContext, System.Guid roomIDNow, RemoteID remoteS, RemoteID userRemote, int userID, long money, bool ipFree, bool shopFree, int shopId)
        {
            ...
            //foreach (var room in RemoteRoomInfos)
            {
                ...
                {
                    if (money < room.baseMoney * 7)
                    {
                        errorMsg = "방이동할 수 있는 최소 금액은 " + (room.baseMoney * 7) + " 입니다.";
                        Proxy.LobbyRoomResponseMoveRoom(remote, rmiContext, false, Guid.Empty, addrTemp, 0, remoteS, userRemote, errorMsg);
                        return false;
                    }
                    // 룸 서버 검색
        ...
        bool RelayRequestLobbyEventInfo(RemoteID remote, CPackOption rmiContext, RemoteID userRemote, CMessage data)
        {
            if (ServerMaintenance)
            {
