...
public class csPopupStorePurchaseInApp : csPopupBase {
    ...
    public override void ShowPopup(object argMSG, System.Action argCloseCB)
    {
        ...
        if (m_Price != null)
        {
            m_Price.text = string.Format("{0}원", m_jsItemInfo.price.ToString("n0"));
        }
    ...
    public void OnClickPurchase()
    {
        ...
        m_bReqPurchase = true;
        //M_NetworkManager.Instance.CheckEnablePurchase(m_jsItemInfo.pid, OnRcvCheckEnablePurchase);
    }
        ...
        if (available)
        {
            if (m_jsItemInfo.purchase_kind == "inapp")
            {
                UnityEngine.Events.UnityAction<bool, string> CBResult = (bool bSuccess, string strMSG) =>
                {
                    ...
                    ClosePopup();
                    //M_NetworkManager.Instance.LobbyIn();
                };
