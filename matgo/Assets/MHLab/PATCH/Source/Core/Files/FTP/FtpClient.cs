        ...
        public static FtpClient Connect(Uri uri, bool checkcertificate) {
            ...
            if (uri.UserInfo != null && uri.UserInfo.Length > 0) {
                if (uri.UserInfo.Contains(":")) {
                    ...
                    cl.Credentials = new NetworkCredential(parts[0], parts[1]);//(HttpUtility.UrlDecode(parts[0]), HttpUtility.UrlDecode(parts[1]));
                }
                else
                    cl.Credentials = new NetworkCredential(uri.UserInfo, "");// HttpUtility.UrlDecode(uri.UserInfo), "");
            }
            else {
